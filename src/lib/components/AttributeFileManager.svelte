<script lang="ts">
	...
	// 업로드 관련 상태
	let selectedUploadFile = $state('attribute.json');
	...
	async function saveSettings(value: boolean) {
		settingsStore.update((settings) => ({
			...settings,
			showAttributeSystemFiles: value
		}));
	}
	...
	async function loadMappingInfo(filename: string) {
		...
		try {
			...
			const result: DbDesignApiResponse = await response.json();
			const mapping = (result.data as { mapping?: { entity?: string; column?: string } } | undefined)
				?.mapping;
	...
	async function saveMappingInfo() {
		...
		try {
			const response = await fetch('/api/attribute/files/mapping', {
				method: 'PUT',
	...
	function handleClose() {
		...
		newFilename = '';
		editingFile = null;
		selectedUploadFile = files.length > 0 ? files[0] : 'attribute.json';
		dispatch('close');
	}
	...
	$effect(() => {
		if (files.length > 0 && !files.includes(selectedUploadFile)) {
			...
			selectedUploadFile = 'attribute.json';
		}
	});
		...
		if (isOpen) {
			...
			const unsubscribe = settingsStore.subscribe((settings) => {
				...
				if (!settingsLoaded) {
					...
					void (async () => {
						await loadFiles();
						await Promise.all([loadEntityFiles(), loadColumnFiles()]);
				...
				{#if activeTab === 'files'}
					<div class="mb-6 rounded-lg border border-gray-200 bg-white/70 p-4">
						...
						<div class="grid gap-3 sm:grid-cols-2">
							<div>
								<label for="attributeEntityMapping" class="block text-xs font-medium text-gray-700">
									엔터티 파일
								</label>
								<select
									id="attributeEntityMapping"
							...
							<div>
								<label for="attributeColumnMapping" class="block text-xs font-medium text-gray-700">
									컬럼 파일
								</label>
								<select
				...
				{:else}
					...
					<div class="space-y-4">
						...
						<div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
							<FileUpload
								disabled={isSubmitting}
								apiEndpoint="/api/attribute/upload"
								contentType="속성 정의서"
