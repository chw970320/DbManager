	...
	try {
		...
		return json({
			...
			data: {
				...
				maxFileSize: '10MB',
				requiredColumns: ['논리DB명', '스키마명', '엔터티명'],
				optionalColumns: ['엔터티설명', '주식별자', '수퍼타입엔터티명', '테이블한글명'],
...
export async function POST({ request, fetch }: RequestEvent) {
	try {
		...
		try {
			parsedEntries = parseEntityXlsxToJson(buffer, !replaceExisting);
		} catch (e) {
		...
		if (replaceExisting) {
			await captureUploadReplaceHistory('entity', filename);
		}
