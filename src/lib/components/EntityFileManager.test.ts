...
vi.mock('$lib/stores/settings-store', () => ({
	settingsStore: {
		subscribe: vi.fn((callback) => {
			callback({ showEntitySystemFiles: false });
			return () => {};
		}),
		update: vi.fn()
	}
...
vi.mock('$lib/utils/file-filter', () => ({
	filterEntityFiles: vi.fn((files: string[], showSystemFiles: boolean) => {
		if (showSystemFiles) return files;
		return files.filter((f) => f !== 'entity.json' && f !== 'history.json');
	})
}));

describe('EntityFileManager', () => {
	beforeEach(() => {
		vi.clearAllMocks();

		// Default fetch mock responses
		mockFetch.mockImplementation((url: string) => {
			if (url.includes('/api/entity/files')) {
				return Promise.resolve({
					ok: true,
					json: () =>
						Promise.resolve({
							success: true,
							data: ['entity.json', 'custom-entity.json']
						})
				});
			}
			return Promise.resolve({
	...
	describe('Rendering', () => {
		...
		it('파일 목록 표시', async () => {
			...
			await waitFor(() => {
				...
				expect(mockFetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/entity/files'),
					expect.any(Object)
				);
			});
		});
		...
		it('매핑 섹션 렌더링 및 매핑 조회 호출', async () => {
			...
			await waitFor(() => {
				expect(mockFetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/entity/files/mapping?filename=')
				);
			});
		});
	});
	...
	describe('File Operations', () => {
		...
		it('파일 이름 변경 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/entity/files') && options?.method === 'PUT') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
				...
				if (url.includes('/api/entity/files') && options?.method === 'GET') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
								success: true,
								data: ['entity.json', 'old-name.json']
							})
					});
				}
				return Promise.resolve({
		...
		it('파일 삭제 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/entity/files') && options?.method === 'DELETE') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
				...
				if (url.includes('/api/entity/files') && options?.method === 'GET') {
					return Promise.resolve({
						...
						json: () =>
							Promise.resolve({
								success: true,
								data: ['entity.json', 'to-delete.json']
							})
					});
				}
				return Promise.resolve({
