<script lang="ts">
	...
	const SYSTEM_FILE = 'attribute.json';
	...
	let activeTab = $state<'files' | 'upload'>('files'); let selectedUploadFile = $state('attribute.json'); let uploadMode = $state<'merge' | 'replace'>('merge');
	...
	async function loadFiles() { isLoading = true; try { const response = await fetch('/api/attribute/files', { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' } }); const result: DbDesignApiResponse = await response.json(); if (result.success && Array.isArray(result.data)) files = result.data as string[]; } catch (_err) { error = '파일 목록을 불러오는데 실패했습니다.'; } finally { isLoading = false; } }
	async function handleCreateFile() { if (!newFilename.trim()) { error = '파일명을 입력하세요.'; return; } const filename = newFilename.trim().endsWith('.json') ? newFilename.trim() : `${newFilename.trim()}.json`; if (files.includes(filename)) { error = '이미 존재하는 파일명입니다.'; return; } isSubmitting = true; error = ''; successMessage = ''; try { const response = await fetch('/api/attribute/files', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename }) }); const result: DbDesignApiResponse = await response.json(); if (result.success) { successMessage = `파일 "${filename}"이(가) 생성되었습니다.`; newFilename = ''; await loadFiles(); dispatch('change'); } else error = result.error || '파일 생성에 실패했습니다.'; } catch (_err) { error = '파일 생성 중 오류가 발생했습니다.'; } finally { isSubmitting = false; } }
	async function handleDeleteFile(filename: string) { if (isSystemFile(filename)) { error = '시스템 파일은 삭제할 수 없습니다.'; return; } if (!confirm(`정말 "${filename}" 파일을 삭제하시겠습니까?`)) return; isSubmitting = true; error = ''; successMessage = ''; try { const response = await fetch('/api/attribute/files', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename }) }); const result: DbDesignApiResponse = await response.json(); if (result.success) { successMessage = `파일 "${filename}"이(가) 삭제되었습니다.`; await loadFiles(); dispatch('change'); } else error = result.error || '파일 삭제에 실패했습니다.'; } catch (_err) { error = '파일 삭제 중 오류가 발생했습니다.'; } finally { isSubmitting = false; } }
	...
	async function saveRename(oldFilename: string) { if (!renameValue.trim()) { error = '파일명을 입력하세요.'; return; } const newFilename = renameValue.trim().endsWith('.json') ? renameValue.trim() : `${renameValue.trim()}.json`; if (newFilename === oldFilename) { cancelRename(); return; } if (files.includes(newFilename)) { error = '이미 존재하는 파일명입니다.'; return; } isSubmitting = true; error = ''; successMessage = ''; try { const response = await fetch('/api/attribute/files', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ oldFilename, newFilename }) }); const result: DbDesignApiResponse = await response.json(); if (result.success) { successMessage = `파일명이 "${newFilename}"(으)로 변경되었습니다.`; cancelRename(); await loadFiles(); dispatch('change'); } else error = result.error || '파일명 변경에 실패했습니다.'; } catch (_err) { error = '파일명 변경 중 오류가 발생했습니다.'; } finally { isSubmitting = false; } }
	function handleFileSelect(filename: string) { attributeStore.update((store) => ({ ...store, selectedFilename: filename })); dispatch('change'); }
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onclick={handleBackdropClick} role="dialog" aria-modal="true">
		<div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl" onclick={(e) => e.stopPropagation()}>
			<div class="sticky top-0 z-10 border-b border-gray-200 bg-white px-6 py-4">
				<div class="flex items-center justify-between"><h2 class="text-xl font-bold text-gray-900">속성 정의서 파일 관리</h2><button onclick={handleClose} class="rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600" aria-label="닫기"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button></div>
