...
import AttributeFileManager from './AttributeFileManager.svelte';
...
vi.mock('$lib/stores/settings-store', () => ({
	settingsStore: {
		subscribe: vi.fn((callback) => {
			callback({ showAttributeSystemFiles: false });
			return () => {};
		}),
		update: vi.fn()
	}
...
vi.mock('$lib/utils/file-filter', () => ({
	filterAttributeFiles: vi.fn((files: string[], showSystemFiles: boolean) => {
		if (showSystemFiles) return files;
		return files.filter((f) => f !== 'attribute.json' && f !== 'history.json');
	})
}));

describe('AttributeFileManager', () => {
	beforeEach(() => {
		vi.clearAllMocks();

		// Default fetch mock responses
		mockFetch.mockImplementation((url: string) => {
			if (url.includes('/api/attribute/files')) {
				return Promise.resolve({
					ok: true,
					json: () =>
						Promise.resolve({
							success: true,
							data: ['attribute.json', 'custom-attribute.json']
						})
				});
			}
			return Promise.resolve({
	...
	describe('Rendering', () => {
		...
		it('파일 목록 표시', async () => {
			...
			await waitFor(() => {
				...
				expect(mockFetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/attribute/files'),
					expect.any(Object)
				);
			});
		});
		...
		it('매핑 섹션 렌더링 및 매핑 조회 호출', async () => {
			...
			await waitFor(() => {
				expect(mockFetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/attribute/files/mapping?filename=')
				);
			});
		});
	});
