...
export async function saveHistoryData(data: HistoryData): Promise<void> {
    try {
        ...
        throw new Error(`히스토리 데이터 저장 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
    }
}
...
export async function loadHistoryData(): Promise<HistoryData> {
    try {
        ...
        throw new Error(`히스토리 데이터 로드 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
    }
}
...
export async function addHistoryLog(newLog: HistoryLogEntry): Promise<HistoryData> {
    try {
        ...
        throw new Error(`히스토리 로그 추가 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
    }
}
