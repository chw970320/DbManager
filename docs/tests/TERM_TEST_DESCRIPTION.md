...
| `term/validate/+server.test.ts`         | 8개       | 완료 |
| `term/sync/+server.test.ts`             | 7개       | 완료 |
| `term/files/+server.test.ts`            | 12개      | 완료 |
| `term/upload/+server.test.ts`           | 4개       | 완료 |
...
| `generator/+server.test.ts`             | 7개       | 완료 |
...
| 테스트명                                      | 설명                 | 검증 내용                                           |
| --------------------------------------------- | -------------------- | --------------------------------------------------- |
| should create a new term entry successfully (mapping success) | 새 용어 생성 성공 (매핑 성공) | 201 응답, UUID 생성, 매핑 상태 true, createdAt/updatedAt 설정 |
| should create a new term entry with mapping failure | 매핑 실패하는 용어 생성 | 매핑 상태 false, unmappedParts 포함                 |
...
| 테스트명                                    | 설명                 | 검증 내용                                |
| ------------------------------------------- | -------------------- | ---------------------------------------- |
| should update an existing entry successfully | 용어 수정 성공       | 200 응답, 매핑 상태 재계산, 필드 업데이트 |
...
## 2. term/validate/+server.test.ts (8개)

**파일 경로**: `src/routes/api/term/validate/+server.test.ts`

용어 유효성 검증 API를 테스트합니다. 용어명 접미사, 중복 검사를 수행합니다.

| 테스트명                                    | 설명                            | 검증 내용                             |
| ------------------------------------------- | ------------------------------- | ------------------------------------- |
| 용어 유효성 검증 성공                       | 유효한 용어 입력                | success=true 반환                     |
| 필수 필드 누락: termName 누락 시 400 에러 | termName 누락                   | 400 에러 반환                         |
| 필수 필드 누락: columnName 누락 시 400 에러 | columnName 누락                 | 400 에러 반환                         |
| 용어명이 2단어 미만인 경우 400 에러를 반환한다 | 단일 단어 입력                  | 400 에러 반환                         |
| 용어명 접미사 validation 실패 시 400 에러를 반환한다 | 접미사 검증 실패                | 400 에러 반환                         |
| 용어명 중복 검사 (신규)                    | 이미 존재하는 termName 입력     | 409 에러 반환                         |
| 용어명 중복 검사 (수정)                    | 자기 자신 제외하고 중복 검사   | entryId 전달 시 자신 제외             |
| should use specified filename parameter     | 파일명 파라미터 사용            | filename 쿼리 파라미터 적용           |
| should use default filename when not specified | 기본 파일명 사용                | 파라미터 없을 때 term.json 사용       |

---

## 3. term/sync/+server.test.ts (7개)
...
| 테스트명                                    | 설명                          | 검증 내용                                        |
| ------------------------------------------- | ----------------------------- | ------------------------------------------------ |
| 매핑 동기화 성공                            | 단어집/도메인 변경 후 동기화  | 매핑 상태가 올바르게 업데이트되는지 확인         |
| 동기화 결과 카운트                          | 업데이트된 항목 수가 정확히 반환 | updated, matchedTerm, matchedColumn, matchedDomain 반환 |
| filename 파라미터 사용                      | filename 파라미터로 특정 파일에서 동기화 수행 | filename 파라미터 적용                           |
| should use default filename when not specified | 기본 파일명 사용              | 파라미터 없을 때 term.json 사용                  |
...
## 4. term/files/+server.test.ts (12개)
...
## 5. term/upload/+server.test.ts (4개)
...
## 6. term/download/+server.test.ts (5개)
...
| should return 404 when no entries exist     | 엔트리 없음             | entries가 비어있을 때 404 에러              |
...
## 7. term/filter-options/+server.test.ts (7개)
...
## 8. term/recommend/+server.test.ts (6개)
...
| 테스트명                                    | 설명                      | 검증 내용                                   |
| ------------------------------------------- | ------------------------- | ------------------------------------------- |
| should return domain recommendations successfully | 도메인 추천 성공          | 200 응답, recommendations 배열 반환         |
| should return empty recommendations when termName is empty | 빈 용어명 처리            | termName이 빈 문자열일 때 빈 배열 반환      |
| should return empty recommendations when last segment not found | 마지막 세그먼트 없음      | 단어집에 없는 단어일 때 빈 배열 반환         |
| should use specified filename parameter      | 파일명 파라미터 사용      | filename 파라미터 적용                      |
| should use default filename when not specified | 기본 파일명 사용          | 파라미터 없을 때 term.json 사용             |
| should return 500 on data load error         | 데이터 로드 오류 처리     | loadTermData 실패 시 500 에러               |
...
## 9. term/validate-all/+server.test.ts (5개)
...
| 테스트명                                    | 설명                      | 검증 내용                                   |
| ------------------------------------------- | ------------------------- | ------------------------------------------- |
| should validate all terms successfully      | 전체 용어 검증 성공       | 200 응답, totalCount, failedCount, passedCount 반환 |
| should return validation errors for invalid entries | 유효하지 않은 항목 검증   | failedEntries 배열에 오류 정보 포함        |
| should use specified filename parameter      | 파일명 파라미터 사용      | filename 파라미터 적용                      |
| should use default filename when not specified | 기본 파일명 사용          | 파라미터 없을 때 term.json 사용             |
| should return 500 on data load error         | 데이터 로드 오류 처리     | loadTermData 실패 시 500 에러               |
...
## 10. generator/+server.test.ts (7개)
...
| 테스트명                                    | 설명                      | 검증 내용                                   |
| ------------------------------------------- | ------------------------- | ------------------------------------------- |
| 용어 조합 생성 성공                         | 한국어 입력 시 가능한 영문 컬럼명 조합 반환 | 200 응답, results 배열 반환                 |
| 단어집에 없는 단어                          | 단어집에 없는 단어 포함 시 빈 배열 또는 에러 반환 | '##'이 포함된 결과 반환                     |
| 한 단어 입력                                | 단일 단어 입력 시 조합 생성 | results 배열 반환                           |
| filename 파라미터 사용                      | filename 파라미터로 특정 단어집 파일 사용 | filename 파라미터 적용                      |
| should use default filename when not specified | 기본 파일명 사용          | 파라미터 없을 때 term.json 사용             |
| should return 400 when term is missing       | term 파라미터 누락        | term 없을 때 400 에러                       |
| should return 500 on vocabulary load error   | 단어집 로드 오류 처리     | loadVocabularyData 실패 시 500 에러         |
...
| 테스트명                                    | 설명                      | 검증 내용            |
| ------------------------------------------- | ------------------------- | -------------------- |
| should display mapping status icons when entry has mapping flags | 매핑 상태 아이콘 표시    | 매핑 상태 표시 확인 |
...
| 테스트명                    | 설명                    | 검증 내용                    |
| --------------------------- | ----------------------- | ---------------------------- |
| 단어집/도메인 파일 매핑 기능 | 단어집/도메인 파일 목록 로드 | `/api/vocabulary/files`, `/api/domain/files` 호출 확인 |
...
| 날짜       | 변경 내용                                    |
| ---------- | -------------------------------------------- |
| 2025-01-09 | 초기 문서 작성 (91개 테스트)                 |
| 2025-01-09 | TermFileManager 컴포넌트 테스트 추가 (100개 테스트) |
