...
- AI Assistant 진입 아이콘을 우하단 플로팅 버튼 배열로 이동했습니다.

### 상세 변경

- AI Assistant 런처를 맨 위로 이동 버튼과 같은 하단 선상에 배치했습니다.
...
- AI Assistant를 오른쪽 drawer에서 좌측 런처 기반 floating/좌측 탭 전환 화면으로 변경했습니다.

### 상세 변경

- AI Assistant 기본 진입점을 좌측 edge 아이콘으로 옮기고, 기본 화면은 좌측 하단 compact floating 창으로 표시합니다.
- 사용자가 좌측 overlay 탭 보기로 전환할 수 있으며 마지막 보기 모드는 브라우저 저장소에 기억됩니다.
- 기존 번들 선택, 출처 표시, 읽기 전용 응답, 사용자 클릭 기반 이동 액션은 유지했습니다.
- Assistant 화면은 기존 최상위 레이어를 유지하되, 오른쪽 검증 패널과 같은 공간을 점유하지 않도록 조정했습니다.
...
- 수동 배치 후 관계선 기호가 기존 Graphviz 방향성과 다르게 보이거나 테이블을 침범하던
  표현을 보정하고, SVG/PNG 다운로드 카드를 기존 본문 상단 위치로 복원했습니다.
- 최초 표시와 배치 수정 후 관계선이 같은 브라우저 오버레이 렌더러를 사용하도록 통일했습니다.
- ERD 미리보기 상단의 그래프 상태, 탐색, 배치 수정 설명 문구를 제거했습니다.
- Docker 빌드 기준 Node.js를 pnpm 11.2.2 요구사항에 맞춰 22.13 이상으로 정렬했습니다.
- 레거시 `registry.json`의 중복 타입 컴포넌트가 공통 파일 매핑 마이그레이션 중 서버 기동을
  중단시키지 않도록 복구 경로를 보강했습니다.
...
- 관계 검증 API 응답, validation report, 정의서 유효성 검사 패널의 상세 데이터와 조치 가이드는
  그대로 유지했습니다.
- `/erd`의 별도 이미지 다운로드 카드를 본문 상단의 테이블 선택 카드 옆에 유지하면서
  ERDViewer의 현재 배치 기준 SVG/PNG 다운로드 액션을 호출하도록 연결했습니다.
- ERD 관계선은 최초 표시부터 브라우저 오버레이에서 crow-foot/tee 기호를 직접 그리고 테이블 경계
  밖에 여백을 둬 기호가 테이블 내부로 들어가지 않게 했습니다.
- 수동 테이블 배치는 서버에 저장하지 않으며 파일, 조건, 선택 테이블, 새로고침 변경 시 초기화됩니다.
- 수동 배치 다운로드는 현재 노드 위치 기준으로 SVG viewBox를 확장해 잘림을 방지하고, 미수정
  PNG도 현재 SVG를 브라우저에서 변환해 화면의 오버레이 관계선과 일치하도록 했습니다.
- Dockerfile은 builder/runner 모두 Node 22 Alpine을 사용하고, 전역 pnpm 설치를 `pnpm@11.2.2`로
  고정하며 `pnpm-workspace.yaml`의 build 승인 정책을 설치 단계에 포함합니다.
- `package.json`, README, 빠른 시작 문서의 Node.js 요구사항을 22.13 이상으로 갱신했습니다.
- 레거시 `registry.json` 관계 그래프가 같은 컴포넌트 안에 같은 타입 파일을 여러 개 포함하면
  파일명 그룹 기준으로 번들을 나눠 v2 공통 매핑을 생성합니다.
- registry에서 이미 복구한 파일은 매핑 필드 없는 데이터 파일의 기본 번들 seed에서 제외해
  마이그레이션 중 자기충돌이 나지 않게 했습니다.
