<script lang="ts">
	...
	const SYSTEM_FILE = 'database.json';
	...
	// 업로드 관련 상태
	let selectedUploadFile = $state('database.json');
	...
	async function saveRename(oldFilename: string) {
		...
		const newFilename = renameValue.trim().endsWith('.json')
			? renameValue.trim()
		...
		try {
			...
			const result: DbDesignApiResponse = await response.json();
			if (result.success) {
				successMessage = `파일명이 "${newFilename}"(으)로 변경되었습니다.`;
				cancelRename();
				await loadFiles();
				dispatch('change');
			} else {
	...
	async function handleUploadSuccess(event: CustomEvent<UploadSuccessDetail>) {
	...
	function handleUploadError(event: CustomEvent<UploadErrorDetail>) {
		error = event.detail.error || '업로드 중 오류가 발생했습니다.';
	}
		...
		>
			...
			<div class="sticky top-0 z-10 border-b border-gray-200 bg-white px-6 py-4">
				<div class="flex items-center justify-between">
					<h2 class="text-xl font-bold text-gray-900">데이터베이스 정의서 파일 관리</h2>
					<button
						onclick={handleClose}
			...
			<div class="p-6">
										...
										{:else}
											...
											>
												...
												{#if isSystemFile(file)}
													<span class="ml-2 rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500"
														>시스템</span
													>
												{/if}
											</button>
				...
				{:else}
					...
					<div class="space-y-4">
						<!-- 대상 파일 선택 -->
						<div>
							<label for="uploadTargetFile" class="mb-1 block text-sm font-medium text-gray-700">
								업로드 대상 파일
							</label>
							<select
								id="uploadTargetFile"
						...
						<div>
							...
							<div class="flex gap-4">
								<label class="flex items-center gap-2">
									...
									/>
									<span class="text-sm">병합 (기존 데이터 유지)</span>
								</label>
								<label class="flex items-center gap-2">
									<input
										type="radio"
										...
										class="text-blue-600 focus:ring-blue-500"
									/>
									<span class="text-sm">덮어쓰기 (기존 데이터 삭제)</span>
								</label>
							</div>
						</div>
