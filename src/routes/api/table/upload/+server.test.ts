...
function createMockRequestEvent(options: {
	...
	const url = new URL('http://localhost/api/table/upload');
...
describe('Table Upload API: /api/table/upload', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	describe('POST', () => {
		it('should upload XLSX file successfully', async () => {
			...
			vi.mocked(mergeTableData).mockResolvedValue({
				entries: [],
				lastUpdated: '2024-01-01T00:00:00.000Z',
				totalCount: 1
			});
		...
		it('should return 400 for invalid content type', async () => {
			...
			const event = {
				request,
				url: new URL('http://localhost/api/table/upload'),
				params: {},
				locals: {},
				platform: undefined,
				route: { id: '/api/table/upload' },
		...
		it('should use specified filename parameter', async () => {
			...
			expect(mergeTableData).toHaveBeenCalled();
		});
	});
});
