...
FileUpload 컴포넌트는 **단순 교체 모드만** 지원합니다.

- 업로드는 항상 교체(`replace=true`) 경로로 처리합니다.
- 업로드 시 검증/동기화 모드를 사용자에게 선택시키지 않습니다.
- 업로드 직전 JSON 본문은 `upload-history`에 저장되며, 30일 동안 복원 가능해야 합니다.
- 업로드 성공 후에는 현재 화면의 매핑값을 자동 저장해야 합니다.
- `업로드 성공 + 매핑 저장 실패`는 묵시적 성공으로 덮지 말고 부분 성공으로 드러내야 합니다.
- FileManager 업로드 탭에는 공통 `UploadHistoryPanel`을 함께 배치해 같은 파일의 교체 이력 조회/복원을 제공하세요.
...
- `onuploadsuccess` 안에서 바로 성공 메시지를 확정하지 말고, 공통 upload orchestration helper로 `업로드 -> 현재 매핑 자동 저장 -> 최종 UI 상태 반영` 순서를 처리합니다.
- 복원은 매핑을 되돌리지 않고 JSON 본문만 되돌립니다.
- 업로드 이력 목록/복원은 `/api/upload-history`, `/api/upload-history/restore`를 사용합니다.
...
### Editor 컴포넌트 닫기 가드 패턴

입력형 Editor 모달은 사용자의 입력 손실을 막기 위해 다음 규칙을 따라야 합니다:

- 배경 클릭으로 닫지 않습니다.
- `닫기(X)`, `취소`, `Escape` 처럼 사용자가 의도한 닫기 경로만 허용합니다.
- 초기값 대비 변경사항이 있으면 `showConfirm`으로
  `작성 취소 확인 / 작성 중인 내용이 사라집니다. 닫을까요?`
  확인 후에만 닫습니다.
- 변경사항이 없으면 바로 닫을 수 있습니다.
- 공통 비교/확인 로직은 `src/lib/utils/editor-close-guard.ts`를 우선 재사용합니다.
...
- 이 저장소의 커밋 메시지는 **항상 한글로 작성**합니다.
- `type`은 영문(`feat`, `fix`, `docs` 등)을 유지하되, `subject`, `body`, `footer` 설명 문장은 한글로 작성합니다.
- 자동화 도구나 에이전트가 커밋할 때도 동일하게 한글 규칙을 적용합니다.
- 커밋 직전에는 실제로 생성할 커밋 메시지를 다시 확인하고, 영어 문장이 남아 있지 않은지 검토합니다.

### 작업 단위 커밋 원칙

- 자동화 도구나 에이전트는 **하나의 작업 단위가 구현, 검증, 문서/TDD 업데이트까지 끝난 시점마다 즉시 별도 커밋**을 생성합니다.
- 여기서 작업 단위란 하나의 의도와 검증 결과로 설명 가능한 최소 변경 묶음을 의미합니다. 큰 작업은 여러 개의 작업 단위 커밋으로 나눕니다.
- 커밋 전에는 최소한 변경 범위에 맞는 테스트/검증과 관련 문서 업데이트를 마친 뒤 커밋합니다.
- 커밋에는 해당 작업 단위와 무관한 사용자 변경을 섞지 않습니다. 안전하게 분리할 수 없으면 억지로 커밋하지 말고 차단 사유를 남깁니다.
- 사용자가 명시적으로 "커밋하지 말라"고 지시한 경우에만 이 기본 원칙을 예외로 둡니다.
...
```
fix: 업로드 대상 파일 기본 선택 동기화

- 파일 업로드 관리 모달의 대상 파일 select 기본값을 현재 선택 파일과 일치시킴
- 공통 선택 로직과 대표 컴포넌트 테스트 추가
```

### 브랜치 운영 및 main 병합 준비

- 기본 작업 브랜치는 `main`입니다.
  - 사용자가 별도 브랜치를 명시적으로 요청하지 않았다면 `main`에서 바로 작업합니다.
  - 자동화 도구나 에이전트도 임의로 기능 브랜치를 생성하지 않습니다.
- 기능 개발을 위해 브랜치를 분기했다면, 작업 완료 기준은 "코드 작성 완료"가 아니라 "`main` 병합 및 브랜치 정리 완료"까지입니다.
- 작업 종료 전에는 최신 `main` 기준이 현재 작업 브랜치에 반영되어 있어야 합니다.
  - 저장소 정책에 맞는 방식으로 `merge` 또는 `rebase`를 사용합니다.
  - 충돌이 발생하면 현재 브랜치에서 해결한 뒤 관련 검증을 다시 수행합니다.
- `main` 기준 반영 후에는 최소한 다음을 다시 확인합니다.
  - `pnpm validate`
  - `pnpm check`
  - 변경 범위에 해당하는 테스트
  - 병합 과정에서 바뀐 동작에 대한 문서 정합성
- 검증이 끝나면 현재 브랜치를 `main`에 병합하고, 병합 확인 후 로컬 브랜치를 삭제합니다.
  - 원격 브랜치가 있으면 함께 삭제합니다.
  - 가능하면 `pnpm run finalize:branch -- <branch-name>` 스크립트로 동일 절차를 수행합니다.
- 네트워크, 권한, 외부 변경 등으로 최신 `main` 반영이나 충돌 해결을 완료할 수 없다면, 작업을 끝낸 것으로 간주하지 말고 차단 사유를 명시적으로 남깁니다.
...
| 1.9.0  | 2026-03 | 커밋 메시지 한글 재확인, `main` 기본 작업 원칙, feature branch 정리 절차 추가 | -      |
| 1.10.0 | 2026-03 | `lint`/`format:check`/`validate` 검증 스크립트 기준 추가                      | -      |
| 1.11.0 | 2026-04 | 작업 단위 완료 후 자동 커밋 원칙과 예외 기준 추가                             | -      |
