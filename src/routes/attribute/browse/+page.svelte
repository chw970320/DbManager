<script lang="ts">
	...
	let allAttributeFiles = $state<string[]>([]);
	let attributeFiles = $state<string[]>([]);
	let selectedFilename = $state('attribute.json');
	...
	onMount(() => {
		...
		unsubscribe = attributeStore.subscribe((value) => {
	...
	async function loadFiles() {
		try {
			...
			if (result.success && Array.isArray(result.data)) {
				...
				allAttributeFiles = files;
				const filteredFiles = filterAttributeFiles(files, showSystemFiles);
				attributeFiles = filteredFiles;
				if (files.length === 0) {
					const createResponse = await fetch('/api/attribute/files', {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
						body: JSON.stringify({ filename: 'attribute.json' })
					});
					if (createResponse.ok) {
						allAttributeFiles = ['attribute.json'];
						attributeFiles = filterAttributeFiles(['attribute.json'], showSystemFiles);
						if (selectedFilename !== 'attribute.json') handleFileSelect('attribute.json');
					}
	...
	async function loadRelationFileMapping(filename: string) {
		try {
			const response = await fetch(
				`/api/attribute/files/mapping?filename=${encodeURIComponent(filename)}`
			);
	...
	async function loadFilterOptions() {
		try {
			...
			const response = await fetch(`/api/attribute/filter-options?${params}`);
			const result: DbDesignApiResponse = await response.json();
	...
	async function handleFileSelect(filename: string) {
		...
		attributeStore.update((store) => ({ ...store, selectedFilename: filename }));
		currentPage = 1;
		searchQuery = '';
	...
	function handleEntryClick(event: { entry: AttributeEntry }) {
		currentEditingEntry = event.entry;
		editorServerError = '';
		showEditor = true;
	}

	async function handleSave(event: CustomEvent<AttributeEntry>) {
		loading = true;
		editorServerError = '';
	...
	async function handleDelete(event: CustomEvent<AttributeEntry>) {
		loading = true;
		editorServerError = '';
		try {
			const params = new URLSearchParams({ id: event.detail.id, filename: selectedFilename });
			const response = await fetch(`/api/attribute?${params}`, { method: 'DELETE' });
			if (response.ok) {
				showEditor = false;
				editorServerError = '';
				currentEditingEntry = null;
	...
	async function handleDownload() {
		...
		try {
			...
			const response = await fetch(`/api/attribute/download?${params}`);
			if (!response.ok) throw new Error(`다운로드 실패: ${response.status}`);
			const blob = await response.blob();
			let filename = 'attribute.xlsx';
...
<svelte:head
	><title>데이터 관리 | 속성 정의서</title><meta
		name="description"
		content="속성 정의서를 관리하고 검색하세요."
	/></svelte:head
>
		...
		<div class="gap-8 lg:grid lg:grid-cols-[16rem_1fr] lg:items-start">
			<aside class="hidden h-full w-64 lg:block">
				...
				>
					...
					<div class="space-y-2">
						{#each attributeFiles as file (file)}<button
								type="button"
								...
								file
									...
									: 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}">{file}</button
							>{/each}{#if attributeFiles.length === 0}<div class="px-4 py-2 text-sm text-gray-500">
								파일이 없습니다.
							</div>{/if}
					</div>
				</div>
			...
			<main class="w-full min-w-0 overflow-x-hidden">
						...
						<div class="mb-4 flex items-center space-x-3">
							...
							<button
								...
								><svg
									...
									/></svg
								><span>{loading ? '준비 중' : 'XLSX 다운로드'}</span></button
							>
							<button
								type="button"
								onclick={refreshData}
								...
								><svg
									...
									/></svg
								><span>{loading ? '로딩 중' : '새로고침'}</span></button
							>
						</div>
					</div>
				</div>
				...
				>
					<div class="mb-6">
						...
						<p class="mt-2 text-gray-600">속성명, 엔터티명 등으로 검색하세요</p>
					</div>
					<div class="mb-6">
						<SearchBar
					...
					<div class="mb-6 flex items-center justify-between">
						<div>
							...
							<p class="mt-1 text-gray-600">
								{#if searchQuery}"{searchQuery}"에 대한 검색 결과 {totalCount.toLocaleString()}건{:else}전체
									속성 정의서 {totalCount.toLocaleString()}건{/if}
							</p>
						</div>
						{#if entries.length > 0}<div class="flex items-center space-x-2 text-sm text-gray-500">
