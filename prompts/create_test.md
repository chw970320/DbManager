...
다음은 @docs/plan/TABLE_TEST_PLAN.md 를 기반으로 TABLE에 대해 테스트 생성해.
...
@docs/ 에 프로젝트는 TDD 기반으로 개발된다는 AI 지침 가이드를 추가해. @docs/AI_CONTEXT.md 파일에 추가하면 좋다고 생각하는데 이외에도 필요하다고 생각되는 지점이 있다면 추가해.
