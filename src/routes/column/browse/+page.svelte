<script lang="ts">
	...
	let selectedFilename = $state('column.json');
	...
	onMount(() => {
		...
		unsubscribe = columnStore.subscribe((value) => {
	...
	async function loadFiles() {
		try {
			...
			if (result.success && Array.isArray(result.data)) {
				...
				allColumnFiles = files;
				const filteredFiles = filterColumnFiles(files, showSystemFiles);
				columnFiles = filteredFiles;
				if (files.length === 0) {
					const createResponse = await fetch('/api/column/files', {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
						body: JSON.stringify({ filename: 'column.json' })
					});
					if (createResponse.ok) {
						allColumnFiles = ['column.json'];
						columnFiles = filterColumnFiles(['column.json'], showSystemFiles);
						if (selectedFilename !== 'column.json') handleFileSelect('column.json');
					}
	...
	async function loadFilterOptions() {
		try {
			...
			const response = await fetch(`/api/column/filter-options?${params}`);
			const result: DbDesignApiResponse = await response.json();
	...
	async function handleFileSelect(filename: string) {
		...
		columnStore.update((store) => ({ ...store, selectedFilename: filename }));
		currentPage = 1;
		searchQuery = '';
		await loadFilterOptions();
		await loadData();
	...
	function handleEntryClick(event: { entry: ColumnEntry }) {
		currentEditingEntry = event.entry;
		editorServerError = '';
		showEditor = true;
	}

	async function handleSave(event: CustomEvent<ColumnEntry>) {
		loading = true;
		editorServerError = '';
	...
	async function handleDelete(event: CustomEvent<ColumnEntry>) {
		loading = true;
		editorServerError = '';
		try {
			const params = new URLSearchParams({ id: event.detail.id, filename: selectedFilename });
			const response = await fetch(`/api/column?${params}`, { method: 'DELETE' });
			if (response.ok) {
				showEditor = false;
				editorServerError = '';
				currentEditingEntry = null;
	...
	async function handleDownload() {
		...
		try {
			...
			const response = await fetch(`/api/column/download?${params}`);
			if (!response.ok) throw new Error(`다운로드 실패: ${response.status}`);
			const blob = await response.blob();
			let filename = 'column.xlsx';
...
<svelte:head
	><title>데이터 관리 | 컬럼 정의서</title><meta
		name="description"
		content="컬럼 정의서를 관리하고 검색하세요."
	/></svelte:head
>
		...
		<div class="gap-8 lg:grid lg:grid-cols-[16rem_1fr] lg:items-start">
			<aside class="hidden h-full w-64 lg:block">
				...
				>
					...
					<div class="space-y-2">
						{#each columnFiles as file (file)}<button
								type="button"
							...
							>{/each}{#if columnFiles.length === 0}<div class="px-4 py-2 text-sm text-gray-500">
								파일이 없습니다.
							</div>{/if}
					</div>
				</div>
						...
						<div>
							...
							<p class="mt-1 text-gray-600">
								{#if searchQuery}"{searchQuery}"에 대한 검색 결과 {totalCount.toLocaleString()}건{:else}전체
									컬럼 정의서 {totalCount.toLocaleString()}건{/if}
							</p>
						</div>
