...
import DatabaseFileManager from './DatabaseFileManager.svelte';
...
vi.mock('$lib/stores/settings-store', () => ({
	settingsStore: {
		subscribe: vi.fn((callback) => {
			callback({ showDatabaseSystemFiles: false });
			return () => {};
		}),
		update: vi.fn()
	}
...
describe('DatabaseFileManager', () => {
	beforeEach(() => {
		...
		mockFetch.mockImplementation((url: string) => {
			if (url.includes('/api/database/files')) {
		...
		it('파일 목록 표시', async () => {
			...
			await waitFor(() => {
				...
				expect(mockFetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/database/files'),
					expect.any(Object)
				);
			});
		});
	...
	describe('File Operations', () => {
		...
		it('파일 이름 변경 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/database/files') && options?.method === 'PUT') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
				...
				if (url.includes('/api/database/files') && options?.method === 'GET') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
								success: true,
								data: ['database.json', 'old-name.json']
							})
					});
				}
				return Promise.resolve({
		...
		it('파일 삭제 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/database/files') && options?.method === 'DELETE') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
				...
				if (url.includes('/api/database/files') && options?.method === 'GET') {
					return Promise.resolve({
						...
						json: () =>
							Promise.resolve({
								success: true,
								data: ['database.json', 'to-delete.json']
							})
					});
				}
				return Promise.resolve({
	...
	describe('Selected File', () => {
		...
		it('업로드 대상 파일이 현재 선택 파일로 기본 설정된다', async () => {
				...
				if (url.includes('/api/database/files')) {
					return Promise.resolve({
						...
						json: () =>
							Promise.resolve({
								success: true,
								data: ['database.json', 'aaa-database.json', 'custom-database.json']
							})
					});
				}
			...
			await waitFor(() => {
				...
				expect(select.value).toBe('custom-database.json');
			});
		});
	});
