<script lang="ts">
	...
	const mappingEndpoint = '/api/column/files/mapping';
	...
	async function saveMappingInfo() {
		...
		try {
			const response = await fetch('/api/column/files/mapping', {
				method: 'PUT',
...
{#if isOpen}
	<div
		...
		role="dialog"
		aria-modal="true"
		aria-labelledby="column-def-file-manager-title"
		tabindex="-1"
	>
		<div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
			...
			<div class="sticky top-0 z-10 border-b border-gray-200 bg-white px-6 py-4">
				<div class="flex items-center justify-between">
					<div>
						<h2 id="column-def-file-manager-title" class="text-xl font-bold text-gray-900">
							컬럼 정의서 파일 관리
						</h2>
