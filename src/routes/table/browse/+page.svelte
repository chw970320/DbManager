<script lang="ts">
	...
	let allTableFiles = $state<string[]>([]); let tableFiles = $state<string[]>([]); let selectedFilename = $state('table.json'); let showSystemFiles = $state(false);
	...
	onMount(() => {
		settingsUnsubscribe = settingsStore.subscribe((settings) => {
			showSystemFiles = settings.showTableSystemFiles ?? false;
	...
	async function loadData() {
		...
		try {
			...
			if (result.success && result.data) { const data = result.data as { entries: TableEntry[]; pagination?: { totalCount: number; totalPages: number } }; entries = data.entries || []; if (data.pagination) { totalCount = data.pagination.totalCount || 0; totalPages = data.pagination.totalPages || 1; } } else { entries = []; totalCount = 0; }
	...
	function handleEntryClick(event: { entry: TableEntry }) { currentEditingEntry = event.entry; editorServerError = ''; showEditor = true; }

	async function handleSave(event: CustomEvent<TableEntry>) { loading = true; editorServerError = ''; const isEditMode = !!currentEditingEntry; try { const params = new URLSearchParams({ filename: selectedFilename }); const response = await fetch(`/api/table?${params}`, { method: isEditMode ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(event.detail) }); const result: DbDesignApiResponse = await response.json(); if (result.success) { showEditor = false; editorServerError = ''; currentEditingEntry = null; await loadData(); } else editorServerError = result.error || (isEditMode ? '수정에 실패했습니다.' : '추가에 실패했습니다.'); } catch (error) { console.error('저장 중 오류:', error); editorServerError = '서버 연결 오류가 발생했습니다.'; } finally { loading = false; } }
	async function handleDelete(event: CustomEvent<TableEntry>) { loading = true; editorServerError = ''; try { const params = new URLSearchParams({ id: event.detail.id, filename: selectedFilename }); const response = await fetch(`/api/table?${params}`, { method: 'DELETE' }); if (response.ok) { showEditor = false; editorServerError = ''; currentEditingEntry = null; await loadData(); } else { const result: DbDesignApiResponse = await response.json(); editorServerError = result.error || '삭제에 실패했습니다.'; } } catch (error) { console.error('삭제 오류:', error); editorServerError = '서버 연결 오류가 발생했습니다.'; } finally { loading = false; } }
	async function handleDownload() { loading = true; try { const params = new URLSearchParams({ filename: selectedFilename }); Object.entries(sortConfig).forEach(([column, direction]) => { if (direction !== null) { params.append('sortBy[]', column); params.append('sortOrder[]', direction); } }); if (searchQuery) { params.set('q', searchQuery); params.set('field', searchField); params.set('exact', searchExact.toString()); } const response = await fetch(`/api/table/download?${params}`); if (!response.ok) throw new Error(`다운로드 실패: ${response.status}`); const blob = await response.blob(); let filename = 'table.xlsx'; const contentDisposition = response.headers.get('Content-Disposition'); if (contentDisposition) { const match = contentDisposition.match(/filename="([^"]+)"/); if (match) filename = match[1]; } const downloadUrl = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = downloadUrl; link.download = filename; link.style.display = 'none'; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(downloadUrl); } catch (error) { console.error('다운로드 중 오류:', error); } finally { loading = false; } }
</script>

<svelte:head><title>데이터 관리 | 테이블 정의서</title><meta name="description" content="테이블 정의서를 관리하고 검색하세요." /></svelte:head>
	...
	<div class="mx-auto w-full px-4 sm:px-6 lg:px-8">
		<div class="gap-8 lg:grid lg:grid-cols-[16rem_1fr] lg:items-start">
			...
			<main class="w-full min-w-0 overflow-x-hidden">
				...
				{#if showEditor}<TableDefEditor entry={currentEditingEntry || {}} isEditMode={!!currentEditingEntry} serverError={editorServerError} filename={selectedFilename} on:save={handleSave} on:delete={handleDelete} on:cancel={() => { showEditor = false; editorServerError = ''; currentEditingEntry = null; }} />{/if}
				<TableDefFileManager isOpen={isFileManagerOpen} on:close={() => (isFileManagerOpen = false)} on:change={async () => { await loadFiles(); await loadData(); }} />

				<div class="mb-8 rounded-2xl border border-gray-200/50 bg-white/80 p-4 shadow-sm backdrop-blur-sm"><div class="mb-6"><h2 class="text-2xl font-bold text-gray-900">통합검색</h2><p class="mt-2 text-gray-600">테이블영문명, 테이블한글명 등으로 검색하세요</p></div><div class="mb-6"><SearchBar bind:query={searchQuery} bind:field={searchField} bind:exact={searchExact} searchFields={[{ value: 'all', label: '전체' }, { value: 'physicalDbName', label: '물리DB명' }, { value: 'schemaName', label: '스키마명' }, { value: 'tableEnglishName', label: '테이블영문명' }, { value: 'tableKoreanName', label: '테이블한글명' }, { value: 'tableType', label: '테이블유형' }, { value: 'subjectArea', label: '주제영역' }]} onsearch={handleSearch} onclear={handleSearchClear} /></div></div>
				<div class="min-w-0 rounded-2xl border border-gray-200/50 bg-white/80 p-8 shadow-sm backdrop-blur-sm"><div class="mb-6 flex items-center justify-between"><div><h2 class="text-2xl font-bold text-gray-900">검색 결과</h2><p class="mt-1 text-gray-600">{#if searchQuery}"{searchQuery}"에 대한 검색 결과 {totalCount.toLocaleString()}건{:else}전체 테이블 정의서 {totalCount.toLocaleString()}건{/if}</p></div>{#if entries.length > 0}<div class="flex items-center space-x-2 text-sm text-gray-500"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><span>페이지 {currentPage} / {totalPages}</span></div>{/if}</div><div class="rounded-xl border border-gray-200"><TableDefTable {entries} {loading} {searchQuery} {totalCount} {currentPage} {totalPages} {pageSize} {sortConfig} {searchField} _selectedFilename={selectedFilename} activeFilters={columnFilters} {filterOptions} onsort={handleSort} onpagechange={handlePageChange} onfilter={handleFilter} onentryclick={handleEntryClick} onClearAllFilters={handleClearAllFilters} /></div></div>
			</main>
		</div>
	</div>
</div>
