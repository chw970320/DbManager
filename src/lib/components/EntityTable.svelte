<script lang="ts">
	...
	type EntryClickEvent = { entry: EntityEntry };
	...
	function handleRowClick(entry: EntityEntry, event: MouseEvent) {
	...
	function getUniqueValues(columnKey: string): string[] {
		...
		entries.forEach((entry: EntityEntry) => {
				...
				{#each Object.entries(activeFilters) as [column, value] (column)}
					{#if value}
						<span class="inline-flex items-center gap-1 rounded-full bg-blue-100 px-3 py-1 text-sm text-blue-800">
							...
							<button onclick={() => handleFilter(column, null)} class="ml-1 hover:text-blue-600" aria-label="필터 제거">×</button>
						</span>
					{/if}
				{/each}
			</div>
	...
	<div class="overflow-x-auto">
		<table class="min-w-full divide-y divide-gray-200">
					...
					{#each columns as column (column.key)}
						<th class="group px-4 py-3 text-xs font-medium uppercase tracking-wider text-gray-500 {column.align === 'center' ? 'text-center' : column.align === 'right' ? 'text-right' : 'text-left'} {column.width}">
							<div class="flex items-center gap-2">
								{#if column.sortable}
									<button onclick={() => handleSort(column.key)} class="flex items-center gap-1 hover:text-gray-900">
										{column.label}
										<span class="text-blue-600">{getSortIcon(column.key)}</span>
										{#if getSortOrder(column.key)}<span class="text-xs text-gray-400">({getSortOrder(column.key)})</span>{/if}
									</button>
								{:else}
								...
								{#if column.filterable}
									<div class="relative">
										<button onclick={() => toggleFilter(column.key)} class="rounded p-1 opacity-0 transition-opacity hover:bg-gray-200 group-hover:opacity-100 {activeFilters[column.key] ? 'opacity-100 text-blue-600' : ''}" aria-label="필터">
											<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
										</button>
										{#if openFilterColumn === column.key}
											<ColumnFilter options={column.filterType === 'select' ? column.filterOptions || filterOptions[column.key] || getUniqueValues(column.key) : filterOptions[column.key] || getUniqueValues(column.key)} value={activeFilters[column.key] || null} type={column.filterType || 'text'} onselect={(value) => handleFilter(column.key, value)} onclose={() => (openFilterColumn = null)} />
										{/if}
									</div>
								{/if}
							</div>
			...
			<tbody class="divide-y divide-gray-200 bg-white">
				{#if loading}
					<tr><td colspan={columns.length} class="px-4 py-8 text-center text-gray-500"><div class="flex items-center justify-center gap-2"><svg class="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>로딩 중...</span></div></td></tr>
				{:else if entries.length === 0}
					<tr><td colspan={columns.length} class="px-4 py-8 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><svg class="h-12 w-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg><p class="text-lg font-medium">데이터가 없습니다</p><p class="text-sm">새 엔터티 정의서를 추가해주세요.</p></div></td></tr>
				{:else}
					{#each entries as entry (entry.id)}
						<tr onclick={(e) => handleRowClick(entry, e)} class="cursor-pointer transition-colors hover:bg-blue-50">
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{@html highlightText(entry.logicalDbName || '', searchField === 'all' || searchField === 'logicalDbName' ? searchQuery : '')}</td>
							...
							<td class="max-w-[300px] truncate px-4 py-3 text-sm text-gray-600" title={entry.entityDescription || ''}>{entry.entityDescription || '-'}</td>
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-600">{entry.primaryIdentifier || '-'}</td>
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-600">{entry.superTypeEntityName || '-'}</td>
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-600">{entry.tableKoreanName || '-'}</td>
						</tr>
					{/each}
				{/if}
			</tbody>
