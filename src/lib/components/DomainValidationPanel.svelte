	...
	function getFilteredResults() {
		...
		if (searchQuery.trim()) {
			...
			filtered = filtered.filter(
				(result) =>
					result.entry.standardDomainName.toLowerCase().includes(query) ||
					result.entry.domainCategory.toLowerCase().includes(query) ||
...
{#if open}
	...
	>
		<div class="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-6 py-4">
			<div>
				<h2 class="text-lg font-semibold text-gray-900">도메인 유효성 검사 결과</h2>
		...
		<div class="border-b border-gray-200 bg-white px-6 py-4">
			<div class="flex flex-col gap-4 sm:flex-row">
				<div class="flex-1">
					<label for="domain-error-filter" class="block text-xs font-medium text-gray-700">오류 유형</label>
					<select
						id="domain-error-filter"
				...
				<div class="flex-1">
					<label for="domain-search" class="block text-xs font-medium text-gray-700">검색</label>
					<input
						id="domain-search"
						type="text"
						bind:value={searchQuery}
						placeholder="도메인명, 분류명, 오류 메시지..."
						class="mt-1 block w-full rounded-md border-gray-300 text-sm"
					/>
				</div>
			</div>
				...
				<div class="space-y-4">
					{#each filteredResults as result (result.entry.id)}
						<div class="rounded-lg border border-gray-200 bg-white p-4">
							<div class="mb-2">
								<div class="font-medium text-gray-900">{result.entry.standardDomainName}</div>
								<div class="text-xs text-gray-600">
									분류: {result.entry.domainCategory} · 타입: {result.entry.physicalDataType}
								</div>
								{#if result.generatedDomainName}
									<div class="text-xs text-blue-700">계산값: {result.generatedDomainName}</div>
								{/if}
							</div>
