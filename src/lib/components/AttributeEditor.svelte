		...
		if (confirm('정말로 이 항목을 삭제하시겠습니까?')) {
			const entryToDelete: AttributeEntry = {
				...
				attributeDescription:
					formData.attributeDescription.trim() || entry.attributeDescription || undefined,
		...
		<div class="flex-1 overflow-y-auto p-6">
			...
			>
				...
				<div class="space-y-4">
					...
					<div>
						...
						><input
							...
							class="w-full rounded-lg border px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 {errors.entityName
								...
								: 'border-gray-300'}"
							placeholder="엔터티명 입력"
						/>{#if errors.entityName}<p class="mt-1 text-xs text-red-500">
								{errors.entityName}
							</p>{/if}
					</div>
					<div>
						<label for="attributeName" class="mb-1 block text-sm font-medium text-gray-700"
							>속성명 <span class="text-red-500">*</span></label
						><input
							id="attributeName"
							type="text"
							bind:value={formData.attributeName}
							class="w-full rounded-lg border px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 {errors.attributeName
								? 'border-red-500'
								: 'border-gray-300'}"
							placeholder="속성명 입력"
						/>{#if errors.attributeName}<p class="mt-1 text-xs text-red-500">
								{errors.attributeName}
							</p>{/if}
					</div>
					<div>
						<label for="attributeType" class="mb-1 block text-sm font-medium text-gray-700"
							>속성유형 <span class="text-red-500">*</span></label
						><input
							id="attributeType"
							type="text"
							bind:value={formData.attributeType}
							class="w-full rounded-lg border px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 {errors.attributeType
								? 'border-red-500'
								: 'border-gray-300'}"
							placeholder="속성유형 입력"
						/>{#if errors.attributeType}<p class="mt-1 text-xs text-red-500">
								{errors.attributeType}
							</p>{/if}
					</div>
					<div>
						<label for="requiredInput" class="mb-1 block text-sm font-medium text-gray-700"
							>필수입력여부</label
						><select
							id="requiredInput"
							bind:value={formData.requiredInput}
					...
					<div>
						<label for="identifierFlag" class="mb-1 block text-sm font-medium text-gray-700"
							>식별자여부</label
						><select
							id="identifierFlag"
							bind:value={formData.identifierFlag}
							class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
							><option value="">선택</option><option value="Y">Y</option><option value="N">N</option
							></select
						>
					...
					<div>
						<label for="refEntityName" class="mb-1 block text-sm font-medium text-gray-700"
							>참조엔터티명</label
						><input
							id="refEntityName"
							type="text"
							bind:value={formData.refEntityName}
							class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
							placeholder="참조엔터티명 입력"
						/>
					</div>
					<div>
						<label for="refAttributeName" class="mb-1 block text-sm font-medium text-gray-700"
							>참조속성명</label
						><input
							id="refAttributeName"
							type="text"
							bind:value={formData.refAttributeName}
							class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
							placeholder="참조속성명 입력"
						/>
					</div>
					<div>
						<label for="attributeDescription" class="mb-1 block text-sm font-medium text-gray-700"
							>속성설명</label
						><textarea
							id="attributeDescription"
							bind:value={formData.attributeDescription}
							rows="3"
							class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
							placeholder="속성설명 입력"
						></textarea>
