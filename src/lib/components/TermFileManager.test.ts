...
vi.mock('$lib/stores/settings-store', () => ({
	settingsStore: {
		subscribe: vi.fn((callback) => {
			callback({ showTermSystemFiles: false });
			return () => {};
		}),
		update: vi.fn()
	}
...
vi.mock('$lib/utils/file-filter', () => ({
	filterTermFiles: vi.fn((files: string[], showSystemFiles: boolean) => {
		if (showSystemFiles) return files;
		return files.filter((f) => f !== 'term.json' && f !== 'history.json');
	})
}));

describe('TermFileManager', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		...
		it('파일 목록 표시', async () => {
			...
			await waitFor(() => {
				...
				expect(mockFetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/term/files'),
					expect.any(Object)
				);
			});
		});
	...
	describe('File Operations', () => {
		...
		it('파일 이름 변경 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/term/files') && options?.method === 'PUT') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
				...
				if (url.includes('/api/term/files') && options?.method === 'GET') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
								success: true,
								data: ['term.json', 'old-name.json']
							})
					});
				}
				return Promise.resolve({
		...
		it('파일 삭제 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/term/files') && options?.method === 'DELETE') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
