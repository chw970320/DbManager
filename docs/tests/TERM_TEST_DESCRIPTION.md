...
용어 browse 화면에서 파일 변경 시 좌측 검색 결과 요약과 용어계 관계 진단 요약이 동시에 갱신 중 상태를 표시하는지 테스트합니다.

| 테스트명                                                                                           | 설명                  | 검증 내용                                                            |
| -------------------------------------------------------------------------------------------------- | --------------------- | -------------------------------------------------------------------- |
| shows loading feedback while file changes refresh the sidebar summary and relationship diagnostics | 파일 전환 로딩 가시화 | 좌측 요약 `aria-busy`, 관계 진단 갱신 문구, 새 파일 기준 요약값 반영 |
