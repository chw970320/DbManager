...
export async function GET({ url }: RequestEvent) {
	try {
		...
		try { tableData = await loadTableData(filename); }
		...
		if (searchQuery.trim()) {
			...
			filteredEntries = tableData.entries.filter((entry) => {
				switch (searchField) {
					...
					default:
						return entry.tableEnglishName?.toLowerCase().includes(query) ||
							entry.tableKoreanName?.toLowerCase().includes(query) ||
							entry.schemaName?.toLowerCase().includes(query) ||
							entry.tableDescription?.toLowerCase().includes(query);
				}
			});
		}
		...
		filteredEntries.sort((a, b) => {
			for (const config of sortConfigs) {
				const aVal = a[config.column as keyof TableEntry], bVal = b[config.column as keyof TableEntry];
		...
		return json({
			success: true,
			data: { entries: paginatedEntries, pagination: { currentPage: page, totalPages, totalCount: filteredEntries.length, limit, hasNextPage: page < totalPages, hasPrevPage: page > 1 }, lastUpdated: tableData.lastUpdated }
...
export async function POST({ request, url }: RequestEvent) {
	try {
		...
		let tableData: TableData;
		try { tableData = await loadTableData(filename); }
		catch { tableData = { entries: [], lastUpdated: new Date().toISOString(), totalCount: 0 }; }

		const now = new Date().toISOString();
		const newEntry: TableEntry = {
			id: uuidv4(),
			physicalDbName: body.physicalDbName || undefined,
			tableOwner: body.tableOwner || undefined,
		...
		return json({ success: true, data: newEntry, message: '테이블 정의서가 성공적으로 추가되었습니다.' } as DbDesignApiResponse, { status: 201 });
	} catch (error) {
...
export async function PUT({ request, url }: RequestEvent) {
	try {
		...
		tableData.entries[idx] = { ...safeMerge(tableData.entries[idx], updateFields), updatedAt: new Date().toISOString() };
		await saveTableData(tableData, filename);

		return json({ success: true, data: tableData.entries[idx], message: '수정 완료' }, { status: 200 });
	} catch (error) {
...
export async function DELETE({ url }: RequestEvent) {
	try {
		...
		const tableData = await loadTableData(filename);
		if (!tableData.entries.find((e) => e.id === id)) return json({ success: false, error: '삭제할 데이터를 찾을 수 없습니다.' }, { status: 404 });
