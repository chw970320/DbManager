<script lang="ts">
	...
	let allColumnFiles = $state<string[]>([]); let columnFiles = $state<string[]>([]); let selectedFilename = $state('column.json'); let showSystemFiles = $state(false);
	let showEditor = $state(false); let editorServerError = $state(''); let isFileManagerOpen = $state(false); let sidebarOpen = $state(false);
	let currentEditingEntry = $state<ColumnEntry | null>(null);
	let unsubscribe: () => void;
	let settingsUnsubscribe: () => void;

	onMount(() => {
		settingsUnsubscribe = settingsStore.subscribe((settings) => {
			showSystemFiles = settings.showColumnSystemFiles ?? false;
			if (allColumnFiles.length > 0) {
				const newFilteredFiles = filterColumnFiles(allColumnFiles, showSystemFiles);
				columnFiles = newFilteredFiles;
		...
		unsubscribe = columnStore.subscribe((value) => { if (selectedFilename !== value.selectedFilename) { selectedFilename = value.selectedFilename; if (browser) { currentPage = 1; searchQuery = ''; loadData(); } } });
		return () => { if (unsubscribe) unsubscribe(); if (settingsUnsubscribe) settingsUnsubscribe(); };
	});

	async function loadFiles() { try { const response = await fetch('/api/column/files'); const result: DbDesignApiResponse = await response.json(); if (result.success && Array.isArray(result.data)) { const files = result.data as string[]; allColumnFiles = files; const filteredFiles = filterColumnFiles(files, showSystemFiles); columnFiles = filteredFiles; if (files.length === 0) { const createResponse = await fetch('/api/column/files', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: 'column.json' }) }); if (createResponse.ok) { allColumnFiles = ['column.json']; columnFiles = filterColumnFiles(['column.json'], showSystemFiles); if (selectedFilename !== 'column.json') handleFileSelect('column.json'); } } else if (filteredFiles.length > 0 && !filteredFiles.includes(selectedFilename)) { handleFileSelect(filteredFiles[0]); } else if (!files.includes(selectedFilename)) { handleFileSelect(filteredFiles.length > 0 ? filteredFiles[0] : files[0]); } } } catch (error) { console.error('파일 목록 로드 오류:', error); } }
	async function loadFilterOptions() { try { const params = new URLSearchParams({ filename: selectedFilename }); const response = await fetch(`/api/column/filter-options?${params}`); const result: DbDesignApiResponse = await response.json(); if (result.success && result.data && typeof result.data === 'object') { filterOptions = result.data as Record<string, string[]>; } } catch (error) { console.error('필터 옵션 로드 오류:', error); } }
	async function handleFileSelect(filename: string) { if (selectedFilename === filename) return; selectedFilename = filename; columnStore.update((store) => ({ ...store, selectedFilename: filename })); currentPage = 1; searchQuery = ''; await loadFilterOptions(); await loadData(); }
	...
	async function loadData() {
		...
		try {
			...
			if (result.success && result.data) { const data = result.data as { entries: ColumnEntry[]; pagination?: { totalCount: number; totalPages: number } }; entries = data.entries || []; if (data.pagination) { totalCount = data.pagination.totalCount || 0; totalPages = data.pagination.totalPages || 1; } } else { entries = []; totalCount = 0; }
	...
	function handleEntryClick(event: { entry: ColumnEntry }) { currentEditingEntry = event.entry; editorServerError = ''; showEditor = true; }

	async function handleSave(event: CustomEvent<ColumnEntry>) { loading = true; editorServerError = ''; const isEditMode = !!currentEditingEntry; try { const params = new URLSearchParams({ filename: selectedFilename }); const response = await fetch(`/api/column?${params}`, { method: isEditMode ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(event.detail) }); const result: DbDesignApiResponse = await response.json(); if (result.success) { showEditor = false; editorServerError = ''; currentEditingEntry = null; await loadData(); } else editorServerError = result.error || (isEditMode ? '수정에 실패했습니다.' : '추가에 실패했습니다.'); } catch (error) { console.error('저장 중 오류:', error); editorServerError = '서버 연결 오류가 발생했습니다.'; } finally { loading = false; } }
	async function handleDelete(event: CustomEvent<ColumnEntry>) { loading = true; editorServerError = ''; try { const params = new URLSearchParams({ id: event.detail.id, filename: selectedFilename }); const response = await fetch(`/api/column?${params}`, { method: 'DELETE' }); if (response.ok) { showEditor = false; editorServerError = ''; currentEditingEntry = null; await loadData(); } else { const result: DbDesignApiResponse = await response.json(); editorServerError = result.error || '삭제에 실패했습니다.'; } } catch (error) { console.error('삭제 오류:', error); editorServerError = '서버 연결 오류가 발생했습니다.'; } finally { loading = false; } }
	async function handleDownload() { loading = true; try { const params = new URLSearchParams({ filename: selectedFilename }); Object.entries(sortConfig).forEach(([column, direction]) => { if (direction !== null) { params.append('sortBy[]', column); params.append('sortOrder[]', direction); } }); if (searchQuery) { params.set('q', searchQuery); params.set('field', searchField); params.set('exact', searchExact.toString()); } const response = await fetch(`/api/column/download?${params}`); if (!response.ok) throw new Error(`다운로드 실패: ${response.status}`); const blob = await response.blob(); let filename = 'column.xlsx'; const contentDisposition = response.headers.get('Content-Disposition'); if (contentDisposition) { const match = contentDisposition.match(/filename="([^"]+)"/); if (match) filename = match[1]; } const downloadUrl = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = downloadUrl; link.download = filename; link.style.display = 'none'; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(downloadUrl); } catch (error) { console.error('다운로드 중 오류:', error); } finally { loading = false; } }
</script>

<svelte:head><title>데이터 관리 | 컬럼 정의서</title><meta name="description" content="컬럼 정의서를 관리하고 검색하세요." /></svelte:head>
		...
		<div class="gap-8 lg:grid lg:grid-cols-[16rem_1fr] lg:items-start">
			<aside class="hidden h-full w-64 lg:block">
				<div class="sticky top-20 rounded-2xl border border-gray-200/50 bg-white/95 p-4 shadow-xl backdrop-blur-md">
					...
					<div class="space-y-2">{#each columnFiles as file (file)}<button type="button" onclick={() => handleFileSelect(file)} class="w-full rounded-lg px-4 py-2 text-left text-sm font-medium transition-colors duration-200 {selectedFilename === file ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-200' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}">{file}</button>{/each}{#if columnFiles.length === 0}<div class="px-4 py-2 text-sm text-gray-500">파일이 없습니다.</div>{/if}</div>
				</div>
			</aside>
			...
			{#if sidebarOpen}
				<div class="fixed inset-0 z-40 flex lg:hidden">
					<div class="w-64 transform bg-white p-4 pt-20 shadow-2xl transition-transform duration-300" role="dialog" aria-modal="true">
						...
						<div class="space-y-2">{#each columnFiles as file (file)}<button type="button" onclick={() => { handleFileSelect(file); sidebarOpen = false; }} class="w-full rounded-lg px-4 py-2 text-left text-sm font-medium transition-colors duration-200 {selectedFilename === file ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-200' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}">{file}</button>{/each}</div>
					</div>
			...
			<main class="w-full min-w-0 overflow-x-hidden">
				...
				{#if showEditor}<ColumnDefEditor entry={currentEditingEntry || {}} isEditMode={!!currentEditingEntry} serverError={editorServerError} filename={selectedFilename} on:save={handleSave} on:delete={handleDelete} on:cancel={() => { showEditor = false; editorServerError = ''; currentEditingEntry = null; }} />{/if}
				<ColumnDefFileManager isOpen={isFileManagerOpen} on:close={() => (isFileManagerOpen = false)} on:change={async () => { await loadFiles(); await loadData(); }} />

				<div class="mb-8 rounded-2xl border border-gray-200/50 bg-white/80 p-4 shadow-sm backdrop-blur-sm"><div class="mb-6"><h2 class="text-2xl font-bold text-gray-900">통합검색</h2><p class="mt-2 text-gray-600">컬럼영문명, 컬럼한글명 등으로 검색하세요</p></div><div class="mb-6"><SearchBar bind:query={searchQuery} bind:field={searchField} bind:exact={searchExact} searchFields={[{ value: 'all', label: '전체' }, { value: 'schemaName', label: '스키마명' }, { value: 'tableEnglishName', label: '테이블영문명' }, { value: 'columnEnglishName', label: '컬럼영문명' }, { value: 'columnKoreanName', label: '컬럼한글명' }, { value: 'dataType', label: '자료타입' }]} onsearch={handleSearch} onclear={handleSearchClear} /></div></div>
				<div class="min-w-0 rounded-2xl border border-gray-200/50 bg-white/80 p-8 shadow-sm backdrop-blur-sm"><div class="mb-6 flex items-center justify-between"><div><h2 class="text-2xl font-bold text-gray-900">검색 결과</h2><p class="mt-1 text-gray-600">{#if searchQuery}"{searchQuery}"에 대한 검색 결과 {totalCount.toLocaleString()}건{:else}전체 컬럼 정의서 {totalCount.toLocaleString()}건{/if}</p></div>{#if entries.length > 0}<div class="flex items-center space-x-2 text-sm text-gray-500"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><span>페이지 {currentPage} / {totalPages}</span></div>{/if}</div><div class="rounded-xl border border-gray-200"><ColumnDefTable {entries} {loading} {searchQuery} {totalCount} {currentPage} {totalPages} {pageSize} {sortConfig} {searchField} _selectedFilename={selectedFilename} activeFilters={columnFilters} {filterOptions} onsort={handleSort} onpagechange={handlePageChange} onfilter={handleFilter} onentryclick={handleEntryClick} onClearAllFilters={handleClearAllFilters} /></div></div>
			</main>
		</div>
	</div>
</div>
