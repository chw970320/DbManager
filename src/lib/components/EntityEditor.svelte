	...
	function handleDelete() {
		...
		if (confirm('정말로 이 항목을 삭제하시겠습니까?')) {
			const entryToDelete: EntityEntry = {
				...
				entityDescription: formData.entityDescription.trim() || entry.entityDescription || undefined,
				primaryIdentifier: formData.primaryIdentifier.trim() || entry.primaryIdentifier || undefined,
