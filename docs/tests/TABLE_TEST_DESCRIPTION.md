# 테이블(Table) 주제영역 테스트 설명

이 문서는 테이블(Table) 주제영역의 모든 테스트 케이스에 대한 상세 설명을 제공합니다.

## 테스트 현황 요약

| 테스트 파일                           | 테스트 수 | 상태 |
| ------------------------------------- | --------- | ---- |
| `table/server.test.ts`                | 17개      | 완료 |
| `table/files/server.test.ts`          | 12개      | 완료 |
| `table/files/mapping/server.test.ts`  | 3개       | 완료 |
| `table/upload/server.test.ts`         | 5개       | 완료 |
| `table/download/server.test.ts`       | 6개       | 완료 |
| `table/filter-options/server.test.ts` | 8개       | 완료 |
| `TableDefEditor.test.ts`              | 11개      | 완료 |
| `TableDefTable.test.ts`               | 5개       | 완료 |
| `TableDefFileManager.test.ts`         | 5개       | 완료 |
| **합계**                              | **72개**  |      |

---

> **추가 회귀 포인트 (2026-03-12)**:
> `table/files/mapping`은 table 화면에서도 `vocabulary/domain/term/database/entity/attribute/column` 전체 연결 상태를 공유 번들로 조회/저장해야 하며,
> `TableDefFileManager`는 다른 7개 파일을 모두 선택할 수 있어야 합니다.

## 1. table/server.test.ts (17개)

**파일 경로**: `src/routes/api/table/server.test.ts`

테이블 CRUD API의 핵심 기능을 검증합니다.

### GET (6개)

| 테스트명                                            | 설명                         | 검증 내용                                  |
| --------------------------------------------------- | ---------------------------- | ------------------------------------------ |
| should return table data successfully               | 테이블 데이터 조회 성공      | 200 응답, success=true, entries 배열 반환  |
...
| should use default filename when not specified      | 기본 파일명 사용             | 파라미터 없을 때 table.json 사용           |

### POST (3개)

| 테스트명                                           | 설명                 | 검증 내용                                                                                                                                    |
| -------------------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| should create a new table entry successfully       | 새 테이블 생성 성공  | 201 응답, UUID 생성, createdAt/updatedAt 설정                                                                                                |
| should return 400 when required fields are missing | 필수 필드 누락       | physicalDbName, tableOwner, subjectArea, schemaName, tableEnglishName, tableKoreanName, tableType, relatedEntityName, publicFlag 누락 시 400 |
...
| 테스트명                                     | 설명                 | 검증 내용                                |
| -------------------------------------------- | -------------------- | ---------------------------------------- |
| should update an existing entry successfully | 테이블 수정 성공     | 200 응답, updatedAt 갱신 확인            |
...
| 테스트명                                     | 설명                 | 검증 내용                                |
| -------------------------------------------- | -------------------- | ---------------------------------------- |
| should delete an existing entry successfully | 테이블 삭제 성공     | 200 응답, 삭제 완료 메시지               |
| should return 400 when id is missing         | ID 누락              | id 파라미터 없이 요청 시 400 에러        |
...
## 2. table/files/server.test.ts (12개)

**파일 경로**: `src/routes/api/table/files/server.test.ts`

테이블 파일 관리 API를 테스트합니다. 파일 목록 조회, 생성, 이름 변경, 삭제 기능을 검증합니다.
...
| 테스트명                                   | 설명              | 검증 내용                         |
| ------------------------------------------ | ----------------- | --------------------------------- |
| should create a new file successfully      | 새 파일 생성 성공 | 201 응답, createTableFile 호출    |
...
| 테스트명                                      | 설명             | 검증 내용                         |
| --------------------------------------------- | ---------------- | --------------------------------- |
| should rename file successfully               | 파일명 변경 성공 | 200 응답, renameTableFile 호출    |
...
| 테스트명                                   | 설명           | 검증 내용                      |
| ------------------------------------------ | -------------- | ------------------------------ |
| should delete file successfully            | 파일 삭제 성공 | 200 응답, deleteTableFile 호출 |
| should return 400 when filename is missing | 파일명 누락    | filename 없이 요청 시 400 에러 |
| should return 500 when delete fails        | 파일 삭제 실패 | 삭제 권한 없음 등 오류 시 500  |

---

### table/files/mapping/server.test.ts (3개)

**파일 경로**: `src/routes/api/table/files/mapping/server.test.ts`
...
## 3. table/upload/server.test.ts (5개)

**파일 경로**: `src/routes/api/table/upload/server.test.ts`

테이블 데이터를 XLSX 파일로 업로드하는 API를 테스트합니다.
...
## 4. table/download/server.test.ts (6개)

**파일 경로**: `src/routes/api/table/download/server.test.ts`

테이블 데이터를 XLSX 파일로 다운로드하는 API를 테스트합니다.
...
| should return 500 on data load error           | 데이터 로드 오류 처리   | loadTableData 실패 시 500 에러              |
| should use specified filename parameter        | 파일명 파라미터 사용    | filename 쿼리 파라미터 적용                 |
| should use default filename when not specified | 기본 파일명 사용        | 파라미터 없을 때 table.json 사용            |

---

## 5. table/filter-options/server.test.ts (8개)

**파일 경로**: `src/routes/api/table/filter-options/server.test.ts`
...
## 6. TableDefEditor.test.ts (11개)

**파일 경로**: `src/lib/components/TableDefEditor.test.ts`

테이블 생성/수정 모달 컴포넌트를 테스트합니다.

### Rendering (5개)

| 테스트명                                                                | 설명                    | 검증 내용                                               |
| ----------------------------------------------------------------------- | ----------------------- | ------------------------------------------------------- |
| should render modal with "새 테이블 정의서" title when not in edit mode | 생성 모드 타이틀        | isEditMode=false 시 "새 테이블 정의서" 표시             |
| should render modal with "테이블 정의서 수정" title when in edit mode   | 수정 모드 타이틀        | isEditMode=true 시 "테이블 정의서 수정" 표시            |
| should display required field labels                                    | 필수 필드 라벨 표시     | 물리DB명, 테이블소유자, 주제영역, 스키마명 등 라벨 존재 |
...
## 7. TableDefTable.test.ts (5개)

**파일 경로**: `src/lib/components/TableDefTable.test.ts`
...
## 8. TableDefFileManager.test.ts (5개)

**파일 경로**: `src/lib/components/TableDefFileManager.test.ts`
...
| 파일 목록 표시                  | 파일 목록 API 호출   | `/api/table/files` 호출 |
| 매핑 섹션 렌더링                | 공통 매핑 UI 표시    | `/api/table/files/mapping` 조회 |
...
# 특정 API 테스트
pnpm test src/routes/api/table/server.test.ts

# 컴포넌트 테스트
pnpm test src/lib/components/TableDefEditor.test.ts
pnpm test src/lib/components/TableDefTable.test.ts
pnpm test src/lib/components/TableDefFileManager.test.ts

# 감시 모드
pnpm test table --watch
```
