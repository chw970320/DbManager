...
- canonical 관계:
  ...
  - `ATTRIBUTE_COLUMN_KEY`: `AttributeEntry(attributeName, identifierFlag, reference entity+attribute)` ↔ `ColumnEntry(columnKoreanName, pkInfo, fkInfo)`. `pkInfo`는 `Y`, `PK`, `PK01` 같은 PK 표시/순번 값을 인정합니다.
...
- 충돌:
  ...
  - 레거시 `registry.json`의 간접 연결 컴포넌트에 같은 타입 파일이 여러 개 섞이면 서버 기동을
    막지 않고 파일명 그룹 기준으로 복구 가능한 번들을 분리합니다.
  - `registry.json`에서 이미 복구한 파일은 매핑 필드가 없는 비기본 데이터 파일의 기본 번들
    seed 대상에서 제외해 마이그레이션 자기충돌을 방지합니다.
...
- 물리 렌더링 표시명: `tableEnglishName`, `columnEnglishName`
- 논리 렌더링은 한글 테이블/컬럼명만 이름 label로 표시하고, 물리 렌더링은 스키마 접두어를 제외한 테이블 영문명과 컬럼 영문명만 이름 label로 표시합니다. 반대쪽 명칭은 보조명으로 섞어 표시하지 않습니다.
...
- `ColumnEntry.fkInfo`가 비어 있으면 관계 edge를 만들지 않습니다.
- `Y/YES/TRUE`는 FK badge/컬럼 marker로만 해석하며 참조 대상이나 관계 edge를 만들지 않습니다.
- `PK01`, `PK02`처럼 PK 순번이 FK 칸에 들어온 값은 업로드 원본의 열 오정렬/순번성 값으로 간주해 FK marker, warning, 관계 edge 모두 만들지 않습니다.
- 정식 관계 형식은 `schema.table.column`입니다.
- `table.column`, `table:column`은 같은 schema 안의 명시적 축약형으로만 해석합니다. 컬럼명만 있는 1-part 값은 관계 추론으로 보지 않고 unresolved warning으로 남깁니다.
...
- 같은 source/target 테이블 사이에 여러 FK가 있어도 Graphviz 이미지 관계는 하나로 축약합니다. 컬럼 단위 FK 정보는 테이블 행의 FK marker로 확인합니다.
- `GraphvizERDRelationship.label`은 기존 관계 요약/호환용 모델 값으로 유지하지만, Graphviz DOT 렌더링은 깔끔한 이미지를 위해 해당 label을 출력하지 않습니다.

### 렌더 출력

- DOT 노드는 HTML-like table label을 사용해 테이블명, PK/FK 좁은 열, 컬럼명, 타입(길이/소수점), NN을 표시합니다.
- 이미지 내부의 그래프 제목, 주제영역 행, 사업범위 문구 행, `PK/FK/컬럼/타입/NN` 머리행, 관계선 라벨은 표시하지 않습니다.
- 사업범위 테이블은 `#4A90E2`, 사업범위 외 테이블은 `#9B9B9B`로 색상만 구분하고, 외부참조 테이블/관계는 회색/점선 스타일을 사용합니다.
- `/api/erd/render?format=svg`는 SVG 이미지, `format=png`는 PNG 이미지를 반환합니다. PNG는 큰 ERD에서 글자 깨짐을 줄이기 위해 기본 192DPI로 렌더링하며, `pngDpi=96..600` 요청 파라미터로 조정할 수 있습니다.
- `/api/erd/generate`의 `metadata.totalEdges`는 기존 ERDData 구조적 edge 수이고, `metadata.totalRelationships`는 Graphviz 이미지 기준 FK 관계 수입니다. ERD 화면은 이미지와 혼동되지 않도록 관계 수를 `totalRelationships` 기준으로 표시합니다.
