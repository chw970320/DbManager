<script lang="ts">
	...
	let props = $props<{
		entries?: TableEntry[]; loading?: boolean; searchQuery?: string; totalCount?: number;
	...
	function getUniqueValues(columnKey: string): string[] {
		...
		entries.forEach((entry: TableEntry) => { const value = entry[columnKey as keyof TableEntry]; if (value !== null && value !== undefined && value !== '') values.add(String(value)); });
	...
	<div class="overflow-x-auto">
		<table class="min-w-full divide-y divide-gray-200">
			...
			<tbody class="divide-y divide-gray-200 bg-white">
				...
				{:else if entries.length === 0}<tr><td colspan={columns.length} class="px-4 py-8 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><svg class="h-12 w-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg><p class="text-lg font-medium">데이터가 없습니다</p><p class="text-sm">새 테이블 정의서를 추가해주세요.</p></div></td></tr>
				{:else}
					{#each entries as entry (entry.id)}
						<tr onclick={(e) => handleRowClick(entry, e)} class="cursor-pointer transition-colors hover:bg-blue-50">
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{@html highlightText(entry.physicalDbName || '', searchField === 'all' || searchField === 'physicalDbName' ? searchQuery : '')}</td>
							...
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{@html highlightText(entry.tableEnglishName || '', searchField === 'all' || searchField === 'tableEnglishName' ? searchQuery : '')}</td>
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{@html highlightText(entry.tableKoreanName || '', searchField === 'all' || searchField === 'tableKoreanName' ? searchQuery : '')}</td>
							<td class="whitespace-nowrap px-4 py-3 text-center text-sm text-gray-600">{entry.tableType || '-'}</td>
							<td class="whitespace-nowrap px-4 py-3 text-sm text-gray-600">{entry.subjectArea || '-'}</td>
							<td class="whitespace-nowrap px-4 py-3 text-center text-sm text-gray-600">{entry.publicFlag || '-'}</td>
							<td class="max-w-[300px] truncate px-4 py-3 text-sm text-gray-600" title={entry.tableDescription || ''}>{entry.tableDescription || '-'}</td>
						</tr>
					{/each}
				{/if}
			</tbody>
