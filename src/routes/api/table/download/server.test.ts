...
describe('Table Download API: /api/table/download', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.mocked(loadTableData).mockResolvedValue(createMockTableData());
		vi.mocked(exportTableToXlsxBuffer).mockReturnValue(Buffer.from('mock-xlsx-data'));
	});
	...
	it('should apply sortBy and sortOrder correctly', async () => {
		const event = createMockRequestEvent({
			searchParams: { sortBy: 'tableEnglishName', sortOrder: 'desc' }
		});

		await GET(event);

		expect(loadTableData).toHaveBeenCalled();
		expect(exportTableToXlsxBuffer).toHaveBeenCalled();
	});

	it('should apply filter correctly', async () => {
		const event = createMockRequestEvent({
			searchParams: { q: 'TABLE1' }
		});
