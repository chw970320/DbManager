...
vi.mock('$lib/stores/settings-store', () => ({
	settingsStore: {
		subscribe: vi.fn((callback) => {
			callback({ showDomainSystemFiles: false });
			return () => {};
		}),
		update: vi.fn()
	}
...
vi.mock('$lib/utils/file-filter', () => ({
	filterDomainFiles: vi.fn((files: string[], showSystemFiles: boolean) => {
		if (showSystemFiles) return files;
		return files.filter((f) => f !== 'domain.json' && f !== 'history.json');
	})
}));

describe('DomainFileManager', () => {
	beforeEach(() => {
		vi.clearAllMocks();
			...
			if (url.includes('/api/domain/files')) {
				return Promise.resolve({
					...
					json: () =>
						Promise.resolve({
							success: true,
							data: ['domain.json', 'custom-domain.json']
						})
				});
			}
			return Promise.resolve({
	...
	describe('File Operations', () => {
		...
		it('파일 이름 변경 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/domain/files') && options?.method === 'PUT') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
				...
				if (url.includes('/api/domain/files') && options?.method === 'GET') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
								success: true,
								data: ['domain.json', 'old-name.json']
							})
					});
				}
				return Promise.resolve({
		...
		it('파일 삭제 기능', async () => {
			mockFetch.mockImplementation((url: string, options?: RequestInit) => {
				if (url.includes('/api/domain/files') && options?.method === 'DELETE') {
					return Promise.resolve({
						ok: true,
						json: () =>
							Promise.resolve({
				...
				if (url.includes('/api/domain/files') && options?.method === 'GET') {
					return Promise.resolve({
						...
						json: () =>
							Promise.resolve({
								success: true,
								data: ['domain.json', 'to-delete.json']
							})
					});
				}
				return Promise.resolve({
