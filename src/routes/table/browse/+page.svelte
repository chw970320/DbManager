<script lang="ts">
	...
	async function loadData() {
		...
		try {
			...
			if (result.success && result.data) { const data = result.data as { entries: TableEntry[] }; entries = data.entries || []; if (result.pagination) { totalCount = result.pagination.totalCount || 0; totalPages = result.pagination.totalPages || 1; } } else { entries = []; totalCount = 0; }
