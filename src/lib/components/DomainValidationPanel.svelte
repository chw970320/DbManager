...
<ValidationPanelShell
	...
	bind:searchQuery
	searchPlaceholder="도메인명, 분류명, 오류 메시지..."
	...
	<div class="space-y-4">
		{#each filteredResults as result (result.entry.id)}
			<div class="rounded-lg border border-gray-200 bg-white p-4">
				<div class="mb-2">
					<div class="font-medium text-content">{result.entry.standardDomainName}</div>
					<div class="text-xs text-content-muted">
						분류: {result.entry.domainCategory} · 타입: {result.entry.physicalDataType}
					</div>
					{#if result.generatedDomainName}
						<div class="text-xs text-brand">계산값: {result.generatedDomainName}</div>
					{/if}
				</div>
