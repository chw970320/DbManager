<script lang="ts">
	...
	// 업로드 관련 상태
	let selectedUploadFile = $state('table.json');
	...
	async function saveMappingInfo() {
		...
		try {
			const response = await fetch('/api/table/files/mapping', {
				method: 'PUT',
	...
	function handleClose() {
		...
		newFilename = '';
		editingFile = null;
		selectedUploadFile = files.length > 0 ? files[0] : 'table.json';
		dispatch('close');
	}
		...
		<div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
			...
			<div class="sticky top-0 z-10 border-b border-gray-200 bg-white px-6 py-4">
				<div class="flex items-center justify-between">
					<h2 class="text-xl font-bold text-gray-900">테이블 정의서 파일 관리</h2>
					<button
						onclick={handleClose}
