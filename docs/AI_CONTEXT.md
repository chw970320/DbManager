...
**"한국어 표준 용어를 기반으로 데이터베이스 용어(컬럼명, 도메인명)를 체계적으로 생성하고, 데이터베이스 설계를 관리하는 시스템"**
...
이 프로젝트는 크게 두 가지 핵심 도메인으로 나뉩니다: **용어 표준화**와 **데이터베이스 설계**. 두 도메인은 현재 아키텍처상 **완전히 분리**되어 있으며, 서로 직접적인 데이터 참조 관계가 없습니다.
...
**역할**: 관리 대상 데이터베이스 인스턴스 정보 관리

#### 5. Entity (엔터티)

**역할**: 데이터베이스의 논리적 데이터 단위인 엔터티 관리

#### 6. Attribute (속성)

**역할**: 엔터티에 포함되는 속성 정보 관리

#### 7. Table (테이블)

**역할**: 물리 데이터베이스에 생성되는 테이블 명세 관리

#### 8. Column (컬럼)

**역할**: 테이블을 구성하는 컬럼의 상세 정보 관리
...
- `static/data/` 디렉토리에 저장 (`vocabulary`, `domain`, `database` 등 기능별 하위 디렉토리 사용)
- 파일별로 독립적인 데이터 관리
...
### 2. 모듈 간 분리 (Decoupling)

**특징**:

- **"용어 표준화"** 모듈(Vocabulary, Domain, Term)과 **"데이터베이스 설계"** 모듈(Database, Entity, Attribute, Table, Column)은 **완전히 분리**되어 있습니다.
- 두 모듈 간에는 데이터 참조, API 호출, UI 통합이 존재하지 않습니다.

**의미**:

- 각 모듈의 독립적인 개발 및 유지보수가 용이합니다.
- 향후 두 모듈을 연계하는 기능(예: 설계 단계에서 표준 용어 추천)을 추가할 수 있는 확장성을 가집니다.
...
- 모든 엔터티(Vocabulary, Domain, Term, Database 등)는 여러 개의 JSON 파일을 지원합니다.
- 파일별 독립적인 데이터 관리
...
- **핵심 타입**: `src/lib/types/`
  ...
  - `database-design.ts`: Database, Entity, Attribute, Table, Column 관련 타입
