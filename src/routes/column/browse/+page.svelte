<script lang="ts">
	...
	const RELATION_SCOPE_TYPE = 'column' as const satisfies DataType;
	...
	function applyInitialBrowseUrlState() {
		...
		if (urlState.filename) {
			...
			columnStore.update((store) => ({ ...store, selectedFilename: urlState.filename }));
		}
		if (urlState.query) {
...
<svelte:head>
	<title>데이터 관리 | 컬럼 정의서</title>
	<meta name="description" content="컬럼 정의서를 관리하고 검색하세요." />
</svelte:head>

{#snippet sidebar()}
	<div class="space-y-4">
		<section
			aria-label="컬럼 정의서 파일 선택"
...
<BrowsePageLayout
	...
	breadcrumbItems={getNavigationBreadcrumbItems('/column/browse')}
...
>
	...
	<BentoGrid>
		<div class="col-span-12">
			<BentoCard title="통합검색" subtitle="컬럼영문명, 컬럼한글명, 도메인명 등으로 검색하세요">
				<SearchBar
		...
		<div class="col-span-12">
			<BentoCard
				title="검색 결과"
				subtitle={searchQuery ? `"${searchQuery}" 검색 결과` : '전체 컬럼 정의서'}
			>
