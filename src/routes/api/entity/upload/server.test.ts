...
import { loadEntityData, mergeEntityData } from '$lib/registry/data-registry';
...
function createMockRequestEvent(options: {
	...
	const url = new URL('http://localhost/api/entity/upload');
...
describe('Entity Upload API: /api/entity/upload', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	describe('POST', () => {
		it('should upload XLSX file successfully', async () => {
			...
			vi.mocked(mergeEntityData).mockResolvedValue({
				entries: [],
				lastUpdated: '2024-01-01T00:00:00.000Z',
				totalCount: 1
			});
		...
		it('should return 400 for invalid content type', async () => {
			...
			const event = {
				request,
				url: new URL('http://localhost/api/entity/upload'),
				params: {},
				locals: {},
				platform: undefined,
				route: { id: '/api/entity/upload' },
		...
		it('should use specified filename parameter', async () => {
			...
			expect(mergeEntityData).toHaveBeenCalled();
		});
	});
});
