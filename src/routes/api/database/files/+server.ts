...
export async function GET(_event: RequestEvent) {
	try {
		const files = await listDatabaseFiles();

		return json(
			{
				success: true,
				data: files,
				message: 'Database files retrieved successfully'
			} as DbDesignApiResponse,
			{ status: 200 }
		);
	} catch (error) {
		console.error('데이터베이스 정의서 파일 목록 조회 중 오류:', error);
...
/**
 * 데이터베이스 정의서 파일 생성 API
 * POST /api/database/files
 */
export async function POST({ request }: RequestEvent) {
	try {
		...
		await createDatabaseFile(filename);
...
/**
 * 데이터베이스 정의서 파일 이름 변경 API
 * PUT /api/database/files
 */
export async function PUT({ request }: RequestEvent) {
	try {
		...
		await renameDatabaseFile(oldFilename, newFilename);

		return json(
			{
				success: true,
				message: 'Database file renamed successfully'
			} as DbDesignApiResponse,
			{ status: 200 }
		);
	} catch (error) {
...
/**
 * 데이터베이스 정의서 파일 삭제 API
 * DELETE /api/database/files
 */
export async function DELETE({ request }: RequestEvent) {
	try {
		...
		await deleteDatabaseFile(filename);
