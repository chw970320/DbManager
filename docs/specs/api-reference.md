...
- 응답 핵심:
  ...
  - `validation.issues`: 미매칭 상세. 각 이슈는 참여 항목 `participants`, 화면 노출 기준
    ...
    `속성 ↔ 컬럼 키`의 `PK정보` 검증은 `Y`, `PK`, `PK01` 같은 PK 표시/순번 값을 PK로 인정합니다.
...
- 주요 파라미터:
  ...
  - `columnFile`: 컬럼 정의서 파일명. 관련 정의서는 공통 파일 매핑으로 해석됩니다.
...
- 응답 핵심:
  ...
  - `data.validationBefore`, `data.validationAfter`: 미리보기 기준 정합성 결과

### POST /api/erd/relations/sync

레거시 5개 정의서 관계 동기화 미리보기를 생성합니다. 상태 변경 apply 경로는 후보 선택 없이 대량 수정하던 legacy 동작이므로 비활성화되었습니다.
