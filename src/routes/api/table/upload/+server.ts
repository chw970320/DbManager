	...
	try {
		...
		return json({
			...
			data: {
				...
				optionalColumns: ['물리DB명', '테이블소유자', '주제영역', '스키마명', '테이블영문명', '테이블한글명', '테이블유형', '관련엔터티명', '테이블설명', '보존기간', '발생주기', '공개/비공개여부'],
...
export async function POST({ request }: RequestEvent) {
	try {
		...
		try { parsedEntries = parseTableXlsxToJson(buffer, !replaceExisting); }
