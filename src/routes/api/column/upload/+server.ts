...
import { loadColumnData, mergeColumnData } from '$lib/registry/data-registry';
	...
	try {
		...
		return json({
			...
			data: {
				...
				maxFileSize: '10MB',
				requiredColumns: ['스키마명', '컬럼영문명', '자료길이', 'PK정보'],
				optionalColumns: [
					'사업범위여부',
					'주제영역',
					'테이블영문명',
...
export async function POST({ request, fetch }: RequestEvent) {
	try {
		...
		try {
			parsedEntries = parseColumnXlsxToJson(buffer, !replaceExisting);
		} catch (e) {
		...
		if (replaceExisting) {
			await captureUploadReplaceHistory('column', filename);
		}
		...
		return json(
			{
				...
				data: {
					...
					postProcess,
					message: `컬럼 정의서 업로드 완료: ${parsedEntries.length}개 항목`
				}
			} as DbDesignApiResponse,
			{ status: 200 }
		);
