<script lang="ts">
	...
	const RELATION_SCOPE_TYPE = 'attribute' as const satisfies DataType;
	...
	function applyInitialBrowseUrlState() {
		...
		if (urlState.filename) {
			...
			attributeStore.update((store) => ({ ...store, selectedFilename: urlState.filename }));
		}
		if (urlState.query) {
...
<svelte:head>
	<title>데이터 관리 | 속성 정의서</title>
	<meta name="description" content="속성 정의서를 관리하고 검색하세요." />
</svelte:head>

{#snippet sidebar()}
	<div class="space-y-4">
		<section
			aria-label="속성 정의서 파일 선택"
...
>
	...
	<BentoGrid>
		<div class="col-span-12">
			<BentoCard title="통합검색" subtitle="속성명, 엔터티명 등으로 검색하세요">
				<SearchBar
		...
		<div class="col-span-12">
			<BentoCard
				title="검색 결과"
				subtitle={searchQuery ? `"${searchQuery}" 검색 결과` : '전체 속성 정의서'}
			>
