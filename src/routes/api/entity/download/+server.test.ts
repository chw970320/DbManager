...
function createMockRequestEvent(options: {
	...
	const url = new URL('http://localhost/api/entity/download');
...
describe('Entity Download API: /api/entity/download', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.mocked(loadEntityData).mockResolvedValue(createMockEntityData());
		vi.mocked(exportEntityToXlsxBuffer).mockReturnValue(Buffer.from('mock-xlsx-data'));
	});
	...
	it('should apply sortBy and sortOrder correctly', async () => {
		const event = createMockRequestEvent({
			searchParams: { sortBy: 'entityName', sortOrder: 'desc' }
		});

		await GET(event);

		expect(loadEntityData).toHaveBeenCalled();
		expect(exportEntityToXlsxBuffer).toHaveBeenCalled();
	});
