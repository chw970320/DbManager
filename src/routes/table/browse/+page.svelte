<script lang="ts">
	...
	let allTableFiles = $state<string[]>([]);
	...
	onMount(() => {
		...
		unsubscribe = tableStore.subscribe((value) => {
	...
	async function loadFiles() {
		try {
			...
			if (result.success && Array.isArray(result.data)) {
				...
				allTableFiles = files;
				const filteredFiles = filterTableFiles(files, showSystemFiles);
				tableFiles = filteredFiles;
				if (files.length === 0) {
					const createResponse = await fetch('/api/table/files', {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
						body: JSON.stringify({ filename: 'table.json' })
					});
					if (createResponse.ok) {
						allTableFiles = ['table.json'];
						tableFiles = filterTableFiles(['table.json'], showSystemFiles);
						if (selectedFilename !== 'table.json') handleFileSelect('table.json');
					}
	...
	async function loadFilterOptions() {
		try {
			...
			const response = await fetch(`/api/table/filter-options?${params}`);
			const result: DbDesignApiResponse = await response.json();
	...
	async function handleSave(event: CustomEvent<TableEntry>) {
		loading = true;
		editorServerError = '';
	...
	async function handleDelete(event: CustomEvent<TableEntry>) {
		loading = true;
		editorServerError = '';
		try {
			const params = new URLSearchParams({ id: event.detail.id, filename: selectedFilename });
			const response = await fetch(`/api/table?${params}`, { method: 'DELETE' });
			if (response.ok) {
				showEditor = false;
				editorServerError = '';
				currentEditingEntry = null;
	...
	async function handleDownload() {
		...
		try {
			...
			const response = await fetch(`/api/table/download?${params}`);
...
<svelte:head
	><title>데이터 관리 | 테이블 정의서</title><meta
		name="description"
		content="테이블 정의서를 관리하고 검색하세요."
	/></svelte:head
>
		...
		<div class="gap-8 lg:grid lg:grid-cols-[16rem_1fr] lg:items-start">
			<aside class="hidden h-full w-64 lg:block">
				...
				>
					...
					<div class="space-y-2">
						{#each tableFiles as file (file)}<button
								type="button"
							...
							>{/each}{#if tableFiles.length === 0}<div class="px-4 py-2 text-sm text-gray-500">
								파일이 없습니다.
							</div>{/if}
					</div>
				</div>
			...
			<main class="w-full min-w-0 overflow-x-hidden">
				...
				>
					<div class="mb-6">
						...
						<p class="mt-2 text-gray-600">테이블영문명, 테이블한글명 등으로 검색하세요</p>
					</div>
					<div class="mb-6">
						<SearchBar
							...
							searchFields={[
								...
								{ value: 'tableType', label: '테이블유형' },
								{ value: 'subjectArea', label: '주제영역' }
							]}
						...
						<div>
							...
							<p class="mt-1 text-gray-600">
								{#if searchQuery}"{searchQuery}"에 대한 검색 결과 {totalCount.toLocaleString()}건{:else}전체
									테이블 정의서 {totalCount.toLocaleString()}건{/if}
							</p>
						</div>
