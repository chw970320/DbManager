<script lang="ts">
	...
	const RELATION_SCOPE_TYPE = 'table' as const satisfies DataType;
	...
	function applyInitialBrowseUrlState() {
		...
		if (urlState.filename) {
			...
			tableStore.update((store) => ({ ...store, selectedFilename: urlState.filename }));
		}
		if (urlState.query) {
...
<svelte:head>
	<title>데이터 관리 | 테이블 정의서</title>
	<meta name="description" content="테이블 정의서를 관리하고 검색하세요." />
</svelte:head>

{#snippet sidebar()}
	<div class="space-y-4">
		<section
			aria-label="테이블 정의서 파일 선택"
...
<BrowsePageLayout
	...
	breadcrumbItems={getNavigationBreadcrumbItems('/table/browse')}
...
>
	...
	<BentoGrid>
		<div class="col-span-12">
			<BentoCard
				title="통합검색"
				subtitle="물리DB명, 스키마명, 테이블영문명, 테이블한글명, 테이블유형, 주제영역으로 검색하세요"
			>
				<SearchBar
		...
		<div class="col-span-12">
			<BentoCard
				title="검색 결과"
				subtitle={searchQuery ? `"${searchQuery}" 검색 결과` : '전체 테이블 정의서'}
			>
